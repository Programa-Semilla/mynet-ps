...
- [x] T004 Verify `pnpm install && pnpm start` brings up a clean database and that the service worker
      registers in dev (007), so nothing here needs a production build
- [x] T005 [P] Move `apps/api/migrations/meta/README.md` aside before any `drizzle-kit generate`, and
      restore it after — `generate` JSON-parses every file in `meta/`
- [x] T006 [P] Confirm the journal's deliberate `0003`/`0004` ordering is untouched after
...
- [x] T007 [P] Create `apps/api/src/db/schema/questions.ts` with `session_questions` per
...
- [x] T008 [P] Add the `CHECK (length(trim(body)) > 0 AND length(body) <= 500)` constraint to
...
- [x] T009 [P] Add indexes on `session_questions(session_id)` and `session_questions(attendee_id)` in
...
- [x] T010 Add `question_votes` to `apps/api/src/db/schema/questions.ts` — composite
...
- [x] T011 Add `question_ids uuid[] NOT NULL` to `apps/api/src/db/schema/reports.ts` (**007's file**)
      ...
      rollback
- [x] T012 Generate migration `0008` covering T007–T011 with `drizzle-kit generate`, having done T005
      first; review the SQL by hand before committing

### Coverage classification — the guards that fail by existence

- [x] T013 Run `apps/api/tests/unit/deletion-coverage.test.ts` and confirm it **fails** on the two
      new tables before classifying them — a guard that was already green proves nothing
- [x] T014 Confirm both new tables pass `deletion-coverage` by their **cascade from `attendees`**,
      requiring **no** allow-list entry and **no** `RETENTION_SWEEPS` rule (FR-765)
- [x] T015 Extend the personal-data export in `apps/api/src/db/queries/account.ts` to cover questions
      asked and votes cast, each naming its session by title (FR-764)
- [x] T016 Cover the new `abuse_reports.question_ids` column in the export alongside `message_ids`,
      and confirm `apps/api/tests/unit/export-coverage.test.ts` passes without an allow-list entry

### Query layer

- [x] T017 Create `apps/api/src/db/queries/questions.ts` with the single list query per
...
- [x] T018 Implement `ask`, `vote` and `unvote` in `apps/api/src/db/queries/questions.ts`, each
...
- [x] T019 Implement `withdraw` in `apps/api/src/db/queries/questions.ts` re-checking
...
- [x] T020 Create `apps/api/src/routes/events/questions.ts` with all five routes under
...
- [x] T021 Append `questionRoutes` to `ROUTES` in `apps/api/src/routes/index.ts` — **append, never
      insert**: the generated contract lists paths in observation order
- [x] T022 Confirm `apps/api/tests/unit/event-scope-audit.test.ts` covers all five routes and that
...
- [x] T023 Regenerate `contracts/openapi.json` and run `pnpm contract:check`; resolve any conflict by
      regenerating wholesale, **never by hand-editing the merged result**

### Throttling

- [x] T024 Add `question_ask` and `question_vote` to `THROTTLE_ACTIONS` in
...
- [x] T025 Add `THRESHOLDS` entries in `apps/api/src/auth/throttle.ts` — `question_ask` 10/100,
      `question_vote` 60/600, both `mayDeny: true` (research R8), each stating why voting is looser
      by an order of magnitude
- [x] T026 Wire both throttles into `apps/api/src/routes/events/questions.ts`

### Repository interface and wiring

- [x] T027 [P] Create `packages/data/src/interfaces/questions.ts` declaring `QuestionsRepository`
      with `eventId` as the **first argument of every method**, mirroring the addresses
- [x] T028 [P] Append `questions: QuestionsRepository` to `Repositories` in
      `packages/data/src/interfaces/index.ts` — one line, no edit to any neighbour's declaration
- [x] T029 Create `packages/data/src/http/questions.ts` implementing `HttpQuestionsRepository`
- [x] T030 Confirm **nothing is added to `CatalogRepository`**, which is read-only in perpetuity and
      asserted by name-shape over its exports (FR-710, FR-772)
- [x] T031 Register `questions` in `apps/web/src/app/services.ts` **undecorated**, with the refusal
...
- [x] T032 Confirm `apps/web/tests/unit/repository-casts.test.ts` still passes with the new member

### Client foundation

- [x] T033 Create `apps/web/src/app/agenda/useSessionQuestions.ts` holding the list, replacing state
      wholesale from each write's response (research R5)
- [x] T034 Create `apps/web/src/app/agenda/PanelQuestions.tsx` as the section shell with its three
...
- [x] T035 Compose `PanelQuestions` in `apps/web/src/app/agenda/SessionPanel.tsx` (**005's file**) —
...
- [x] T036 [P] [US1] Integration test in `apps/api/tests/integration/questions-ask.test.ts` — a
...
- [x] T037 [P] [US1] Integration test in `apps/api/tests/integration/questions-validation.test.ts` —
      empty, whitespace-only and over-500 bodies are refused at the route **and** by the `CHECK`
      constraint independently
- [x] T038 [P] [US1] Component test in `apps/web/tests/component/panel-questions-ask.test.tsx` — the
      post control is **disabled** for empty and whitespace input, and the remaining allowance
      appears before the limit is reached
- [x] T039 [P] [US1] Component test asserting the empty state invites the first question rather than
      rendering a blank area

### Implementation for User Story 1

- [x] T040 [US1] Implement the ask form in `apps/web/src/app/agenda/PanelQuestions.tsx` — disabled
      post control (FR-704), visible remaining allowance (FR-706), field clears on success (FR-707) (FR-701, SC-701, SC-702)
- [x] T041 [US1] Render the attributed list in `apps/web/src/app/agenda/PanelQuestions.tsx` showing
      the author's display name for **every** question, including a non-discoverable author's (FR-702, FR-708, FR-734, SC-703)
- [x] T042 [US1] Ensure opening an author's profile from a question goes through the **existing**
      profile rules unchanged, so a non-discoverable author's name shows and their profile still
      refuses to open (FR-736)
- [x] T043 [US1] Tell the attendee, at the point of asking, that their question will carry their name
      to everyone at the session (FR-739) — a consequence they must not meet afterwards
- [x] T043a [US1] **Classify every refusal in `apps/web/src/app/agenda/useSessionQuestions.ts` on
...
- [x] T043b [P] [US1] Component test in `apps/web/tests/component/panel-questions-errors.test.tsx`
...
- [x] T044 [P] [US2] Integration test in `apps/api/tests/integration/questions-vote.test.ts` — one
      vote per attendee enforced by the schema; a repeated request is idempotent; withdrawal returns
      the count exactly (SC-704)
- [x] T045 [P] [US2] Integration test asserting an attendee **cannot vote on their own question**
      (FR-722) and that the refusal carries its reason
- [x] T046 [P] [US2] Integration test asserting two readers with the same data receive the **same
      order**, including where counts tie (SC-705)
- [x] T047 [P] [US2] Component test in `apps/web/tests/component/panel-questions-vote.test.tsx` — the
      voted state renders, and no surface anywhere names another voter (FR-721)

### Implementation for User Story 2

- [x] T048 [US2] Implement the upvote control in `apps/web/src/app/agenda/PanelQuestions.tsx` with an
      accessible label naming its question and a pressed state conveyed to assistive technology (FR-720, FR-724, SC-706)
- [x] T049 [US2] Key list rows on the question id so a row that **moves does not unmount** — this is
      what keeps focus on the control the reader just activated (FR-780, research R5)
- [x] T050 [US2] Hide the upvote control on the reader's own question, matching the server's refusal
...
- [x] T051 [P] [US3] Integration test in `apps/api/tests/integration/questions-withdraw.test.ts` —
...
- [x] T052 [P] [US3] Integration test for the **race**: a vote cast between render and submission
      causes the transaction-internal re-check to refuse (FR-714)
- [x] T053 [P] [US3] Integration test asserting withdrawal becomes available again when every vote is
      withdrawn (Edge Cases)
- [x] T054 [P] [US3] Component test asserting the control is **absent with the reason stated** rather
      than present-and-failing (FR-713)

### Implementation for User Story 3

- [x] T055 [US3] Implement the withdrawal control in `apps/web/src/app/agenda/PanelQuestions.tsx`,
      driven by the server's `canWithdraw` while the server remains the enforcement (FR-713)
- [x] T056 [US3] Wire the confirmation through the existing `ConfirmDialog`, opened **inside** the
...
- [x] T057 [US3] Confirm the confirmation adds **no local centring class** — `theme/tokens.css`
...
### Tests for User Story 4
 (FR-779)
- [x] T058 [P] [US4] Integration test in `apps/api/tests/integration/questions-deletion.test.ts` —
...
- [x] T059 [P] [US4] Integration test asserting **no placeholder, no de-attributed row and no
      tombstone** survives anywhere (owner decision 1)
- [x] T060 [P] [US4] Integration test in `apps/api/tests/integration/questions-export.test.ts` — the
      export contains every question asked and every vote cast, each naming its session (SC-710)
- [x] T061 [P] [US4] Integration test asserting a session whose only question was the departing
      attendee's renders the **empty state**, not a gap or an error

### Implementation for User Story 4

- [x] T062 [US4] Confirm deletion needs **no application code at all** — both cascades are
      schema-level from T007 and T010, which is the point of putting them there
- [x] T063 [US4] Verify the re-seed path: `DELETE FROM events` must succeed with questions present
...
- [x] T064 [P] [US5] Integration test in `apps/api/tests/integration/questions-blocks.test.ts` — a
...
- [x] T065 [P] [US5] Integration test asserting a block changes **no other reader's list and no
      count at all**, including a vote the blocker had already cast (FR-787, SC-711b)
- [x] T066 [P] [US5] Integration test asserting unblocking restores visibility **with no write**
      (FR-786) — the deliberate opposite of 008's appointment cancellation
- [x] T067 [P] [US5] Integration test in `apps/api/tests/integration/reports-questions.test.ts` — a
...
- [x] T068 [P] [US5] Confirm `apps/api/tests/unit/no-report-read-surface.test.ts` passes
      **unmodified** with the new field

### Implementation for User Story 5
 (FR-773a)
- [x] T069 [US5] Add the bidirectional `NOT EXISTS` block filter to the list query in
      `apps/api/src/db/queries/questions.ts`, mirroring `apps/api/src/db/queries/cards.ts` (FR-785)
- [x] T070 [US5] Accept and store `questionIds` in `apps/api/src/routes/reports.ts` (**007's file**) (FR-783)
- [x] T071 [US5] Add `report_submit` to `THROTTLE_ACTIONS` and `THRESHOLDS` (5/50, `mayDeny: true`)
...
- [x] T074 [US5] Confirm the report dialog's confirmation is **disabled while the reason is blank**,
...
### Layouts — the three widths, none of them optional
 (FR-782, FR-784)
- [x] T075 [P] Desktop: the panel stays centred with the section beneath the existing three;
      nothing scrolls horizontally (FR-774)
- [x] T076 [P] Tablet: reduced rail, constrained panel width, **single-column** question list —
      questions are prose and a two-column list of prose is harder to scan
- [x] T077 [P] Mobile at 390×844: panel full-width from the bottom edge, vote and report controls at
      touch size, a long question **wraps** (FR-775) (FR-776)
- [x] T078 Extend `e2e/responsive.spec.ts` to measure that **both** dialogs this feature opens are
      centred at every width — the assertion exists because two dialogs shipped in the corner, and
      this feature opens two more

### Accessibility

- [x] T079 [P] Accessible labels on every control; the upvote names its question and conveys pressed
      state
- [x] T080 [P] Visible focus on every control, and the whole section operable by keyboard alone (FR-777, SC-713)
- [x] T081 E2E in `e2e/session-qa-keyboard.spec.ts`: **Escape dismisses only the inner dialog**,
...
- [x] T082 E2E asserting focus **stays on the upvote control as its row moves up the list** (FR-780)
      — the assertion most likely to fail
- [x] T083 Extend `e2e/accessibility` coverage to the Q&A section

### Requirements whose implementation is an absence

- [x] T084 [P] Confirm `apps/api/tests/unit/notification-triggers.test.ts` passes **unmodified** —
...
- [x] T085 [P] Assert by test the absence of any question edit route or column (FR-709, FR-770)
- [x] T086 [P] Assert by test the absence of any downvote or reaction (FR-723, FR-771)
- [x] T087 [P] Assert by test that no route or repository method reveals **who** voted (FR-721,
      FR-769)
- [x] T088 [P] Assert by test the absence of any answer, `answered` flag, pin or moderation route
      (FR-768) — each is an organizer act
- [x] T089 [P] Assert by test that no Home card and no navigation destination is added (FR-753,
      FR-773)
- [x] T090 [P] Assert by test that no de-duplication or "already asked" refusal exists (FR-773b)
- [x] T090a [P] Assert by test that **nothing in this feature polls** (FR-731) — no interval, no
...
- [x] T090b [P] Assert over `apps/api/src` that **no contact, connection or relationship is derived
...
- [x] T090c [P] Assert by test that no notification bell and no in-app notification centre is
...
- [x] T091 [P] SC-711: read a cached programme, ask a question and vote, read it again — **the
...
- [x] T092 [P] SC-712: every write refused offline with an explanation, **and the typed text still
      present** (FR-759)
- [x] T093 [P] SC-707: a non-discoverable author is named on their question **and** their profile
      still refuses to open — both halves together, because either alone looks like a defect
- [x] T094 [P] SC-715: a Q&A failure leaves overview, speakers and notes rendered and interactive

### Documentation and closing

- [x] T095 [P] Update `CLAUDE.md` — current state, the Q&A architectural invariants, and the
      standing decisions this feature settled
- [x] T096 [P] Record in `docs/superpowers/specs/2026-08-06-mynet-delivery-roadmap-design.md` that
      009 shipped and that **the roadmap is complete**
- [ ] T097 **NOT DONE — carried forward, as 007's and 008's were.** This needs a person at a
      browser and cannot be discharged from here. What *is* covered: `e2e/session-qa.spec.ts`
      walks two real browser profiles through asking, seeing, attributing and upvoting, measures
      both nested dialogs at three widths, and drives the Escape and focus behaviours; the
      accessibility spec scans the populated section and the nested dialog. What is **not**
      covered is a human looking at it — which is exactly how 008's top-left dialog was found
      after passing every gate. Scenario 6 is the one that must not be skipped.
      **Walk `quickstart.md` by hand, all eight scenarios, in two browser profiles.** T148's
...
- [x] T098 Run every gate: `pnpm typecheck && pnpm lint && pnpm test:unit && pnpm test:component &&
      ...
      → **`pnpm verify` exit 0**, which runs all ten gates in one command. From its output, not
      from inspection: typecheck clean across five packages · lint clean at `--max-warnings=0` ·
      format clean · **unit 416 passed** (45 files) · **component 554 passed** (56 files) ·
      contract up to date · **integration 897 passed** (103 files) · production build · asset
      budget 96.4 KB of 150 KB · **e2e 141 passed**

...
**Status**: revised after the plan gate returned **NOT READY** (5 Critical, 10 Important, 7 Minor)
...
- [ ] T001 Take a verified `pg_dump` of UAT before anything destructive, or verify the existing `deploy/vm/backups/pre-014-manual-20260816T012211Z.dump` restores into a throwaway container; record the path in `deploy/vm/OPERATIONS-LOG.md`
- [ ] T002 [P] Extract the operator insert into an exported `ensureOperatorIdentities(db)` in `apps/api/src/db/seed/operators.ts`, using `.onConflictDoNothing({ target: operators.email })` and keeping the FR-918 cross-table address check
...
- [ ] T007 Re-seed UAT (7 attendee rows, of which 4 are genuine and disposable), run `admin:seed-operators`, issue a credential, confirm sign-in at `admin.mynet-dev.programasemilla.com` (FR-1100, FR-1101, SC-1209)
...
- [ ] T013 [P] Correct the FR-902 citation in `apps/api/tests/integration/admin-bootstrap.test.ts` (lines ~112, ~118, ~129). **`apps/api/tests/unit/admin-forbidden-surfaces.test.ts`'s four FR-902 references are CORRECT and must not be touched**
...
- [ ] T018 Add an e2e assertion in `e2e/messages-journey.spec.ts` that Messages reaches a terminal state — content **or** error — in every declared engine

### Three browser engines (FR-1144)

- [ ] T019 Add `firefox` and `webkit` projects to `playwright.config.ts`, restricted to `responsive.spec.ts` and `first-viewport.spec.ts`. **Guard the `capturingScreenshots` ternary** — a project-level `testMatch` overrides the top-level one, silently defeating the `CAPTURE_SCREENSHOTS` switch
...
- [ ] T021 Change the browser install line in `.github/workflows/verify.yml` to `chromium webkit firefox`. **Add no job** — `verify` and `verify-push` hard-assert a count of 10
...
- [ ] T025 Change `test-e2e`'s spec derivation in `.github/workflows/verify.yml` to recurse (`find e2e -name '*.spec.ts'`) with the accessibility exclusion re-anchored to **path** rather than suffix, so `e2e/accessibility/identity.spec.ts` is picked up. Keep the emptiness guard
...
- [ ] T029 Install the backup cron on UAT and confirm a scheduled backup actually fires
- [ ] T030 **Exercise a restore on the real host** and record it in `OPERATIONS-LOG.md`. This is 011's undischarged T081, paid off here — walk scenarios 006/3d and 011/8 fail at step 1 without it

### The offline disclosure (FR-1140, FR-1140a, FR-1149)

*T031 comes FIRST. FR-1131 forbids fixing-then-recording.*

- [ ] T031 **Record the decision before making the change**: closing SC-1207 narrows FR-215's field behaviour — an offline PWA cold start no longer resolves an identity. Weigh v3.0.0's and v5.0.0's precedent explicitly and state why this is MINOR rather than a retraction needing an amendment. Owner-signed, in `specs/012-launch-readiness/decisions.md` (FR-1131, FR-1136)
...
- [ ] T045 Enumerate every by-hand scenario across all 13 `specs/*/quickstart.md`. **Do not key on heading shape** — that is what hid 006. Phase 0 measured 122 scenarios, 74 outstanding walk units, 90 unaccounted
- [ ] T046 Give 002's 8 and 004's 10 scenarios an explicit disposition — they have **no walk task at all**, so "not marked complete" cannot be evaluated against them (SC-1202)
...
- [ ] T049 Pick **one** width triple, record why, **and update `spec.md` US4 scenario 1 and SC-1204 to match**. Thirteen places assert six inconsistent triples; the sweep uses a seventh; the spec named an eighth
...
- [ ] T057 Resolve how UAT receives a build: create the `AZURE_CREDENTIALS` service principal, **or** record an explicit decision that UAT is deployed by hand from `develop` via `deploy/vm/deploy.sh` and amend FR-1120a to say so. **CI has never deployed anything** — `deploy-uat` short-circuits on the unset secret
- [ ] T058 Deploy `develop` to UAT and record the commit SHA in `OPERATIONS-LOG.md` (FR-1120a)
- [ ] T059 **Name who plants the seeded defects, and confirm they are not the walker.** If no second person exists, execute T060 instead and mark FR-1127 **unmet** in the Feature Declarations rather than letting it read as satisfied
- [ ] T060 *(fallback, only if T059 finds no second person)* Nominate who performs the 10% spot-check of passed steps before Phase 8, and record that FR-1127's floor was not available
- [ ] T061 Plant **three defects** — one layout, one copy, one refusal message — in a throwaway build deployed to UAT, recorded as *base commit + named seed patch*. **Not `develop`** (FR-1127)

---

## Phase 5 — Walk it

**Sequenced, not parallel.** Each phase consumes the previous one's output, and all four share one
...
- [ ] T074 [US1] Walk the mutual card exchange in two profiles and **read the confirmation copy** — where 016's FR-1055 defect lived with two green tests requiring the wrong sentence
...
- [ ] T079 [US4] Walk `apps/admin` at the chosen widths with captures — **never reviewed at any width**
...
- [ ] T085 [US5] Install on a **physical Android phone**; exercise `beforeinstallprompt`, the install banner and **real Web Push delivery** — every push walk to date has been desktop only
...
- [ ] T089 Reveal the three seeded defects. **A miss voids the walk and it is re-done** (FR-1127, SC-1208). Borrowed from 001's still-open T093: *a gate that does not fail when broken is not a gate*
...
- [ ] T097 Add a gate asserting every step in `quickstart.md` has a row in `walk-record.md`, and every layout and install row names a capture file **that exists on disk**. Without it the human gate has no machine-visible output (SC-1203, FR-1126)
- [ ] T098 Mark discharged walk tasks complete in each feature's own `tasks.md`, **feature-prefixed**: `001/T093`, `006/T080`, `006/T081`, `006/T102`, `007/T148`, `008/T148`, `008/T149`, `009/T097`, `010/T066`–`010/T070`, `011/T073`, `011/T074`, `011/T082`, `011/T084`, `013/T158`, `013/T159`, `014/T105`, `014/T206`, `014/T207`, `016/T067`, `016/T068`. Annotate each as **"discharged by 012"** rather than silently ticking it
...
- **T002, T005, T006, T009, T011, T012, T013, T014** — different files

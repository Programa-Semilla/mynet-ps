...
| Environment note (not an observation) | Before Part C, 'Grace Conference 2' was moved by its organizer through the admin API from 17–19 Aug to **25–27 Sep 2026 (UTC)**, its two remaining sessions with it (session 1: 25 Sep 18:00–19:00 UTC; session 3: 26 Sep 16:00–18:00 UTC), so the conference day is today. Moving session 3 is a material change: its saver received a 'session moved' push and carries a changed-marker on that row — expected, caused by the move, not by Part C. The organizer then authored five more sessions through the admin API (2026-09-25), since B7 had left two: Opening keynote (25 Sep 15:00Z), Designing for offline (25 Sep 20:00Z), Open networking hour (25 Sep 22:00Z, no speaker), Accessibility in practice (26 Sep 19:00Z), Closing panel (27 Sep 17:00Z) — seven sessions across three days, all mandatory. |
...
| B6 | Cancellation, and the one surface that omits it | Cancellation dispatched its notification ('This session has been cancelled'); the Agenda row carries the cancelled label; the walker confirmed Up next skips the cancelled session while the day's list still shows it — the one-surface asymmetry present. (Session 3 was later reinstated to probe B7's fence; cancelled rows offer Reinstate instead of Cancel-or-delete.) | pass |  |
| B7 | Delete is fenced by engagement | (a) untouched throwaway deleted without ceremony (UI, DELETE 204 — first two rounds of this step reported 'worked' while the server showed no request; the scripted UI run is the evidenced walk). (b) engaged session: delete NOT OFFERED — dialog reads 'Attendees have already engaged with this session. Cancelling keeps everything they wrote; deleting is not available while any of it exists', itemising 1 saved / 0 notes / 0 questions / 0 votes; only Cancel or Keep offered. (c) optional session with a held place: delete allowed behind the full ratified-cost sentence — holders 'will not be notified… no marker… they find out by arriving', cancel offered as the preserving alternative; confirmed, DELETE 204 with a placesSeen guard, session gone. Dialog stated 1 place held at that moment. | pass | b7b-engaged-refusal.png b7c-held-places-dialog.png b7c-after-delete.png |
...
| B9 | The roster, and what it must not show | The roster names every enrolled attendee and offers nothing clickable — no profile route, nothing beyond the names (walked before the (c) deletion destroyed the roster with the session). | pass |  |

...
- [ ] T002 `git mv seeds/logo.png assets/brand/logo.png` and commit it as tracked (FR-800, FR-801) —
      it is currently untracked
- [ ] T003 Remove the now-empty `seeds/` directory (FR-802) — an empty directory named for database
      seed data is the collision the move exists to remove
- [ ] T004 Add `sharp` (`^0.35.3`, matching `apps/api`) to **root** `devDependencies` in
...
- [ ] T005 [P] Grep the repository for `seeds/logo.png` and confirm **zero** remaining references
...
- [ ] T006 Create `scripts/generate-brand-assets.mjs` with its constants stated as readable values
      at the top: expected board dimensions `1254×1254`, crop `{left:172, top:162, width:283,
      height:300}`, plate `#0d1942`, mark `#fe6551` (FR-805)
- [ ] T007 In the same file, write the reasoning for the brand constants beside their definition
...
- [ ] T008 Add a source assertion that reads the board's real dimensions and **fails loudly** naming
      what it expected, rather than cropping a meaningless rectangle from a different image (FR-807)
- [ ] T009 Implement the alpha matte: `α = clamp((P.red − 0x0d) / 241, 0, 1)` per pixel, RGB set to
...
- [ ] T010 Implement `plate(matte, size, markHeight)` — resample the matte with a Lanczos filter and
      centre it on an **opaque** square of the plate colour
- [ ] T011 Implement `bare(matte, height, colour)` — resample onto **transparency**, painting the
      matte any colour, for the in-app marks (FR-820a)
- [ ] T012 Implement `maskableMarkHeight(size)` returning `2 × (0.8 × size / 2) / hypot(283/300, 1)`
...
- [ ] T013 [P] Create `scripts/generate-brand-assets.test.mjs` asserting the geometry: at 512 the
...
- [ ] T014 [P] Extend that test with the unmix maths: α is 0 at the plate colour, 1 at the mark
      colour, and 0.5 at their midpoint
- [ ] T015 Wire the CLI entry point, create output directories, and make every write go through one
...
- [ ] T016 [US1] Generate `apps/web/public/icons/icon-192.png` at 192×192 on the opaque plate, replacing
      the provisional file **at the same name and size** (FR-810)
- [ ] T017 [US1] Generate `apps/web/public/icons/icon-512.png` at 512×512, same name and size (FR-810).
      No safe-zone constraint applies — the fill fraction is a design choice (research R1)
- [ ] T018 [US1] Generate `apps/web/public/icons/icon-maskable-512.png` using `maskableMarkHeight(512)`
      (FR-811). **Do not size the mark to 80% of the side**
- [ ] T019 [US1] Generate `apps/web/public/apple-touch-icon.png` at 180×180 with a **fully opaque** plate
...
- [ ] T020 [P] [US1] Add an assertion to `scripts/generate-brand-assets.test.mjs` that the apple-touch
      asset contains **zero** pixels with alpha < 255 (SC-803)
- [ ] T021 [US1] Commit the four generated assets (FR-808) and confirm none carries the placeholder's
      disc or amber diagonal band (FR-814, SC-801)
- [ ] T022 [US1] Render the maskable icon through a circular mask locally and confirm **both node
...
- [ ] T023 [P] [US2] Generate `apps/web/public/favicon-32.png` and `favicon-16.png` on the opaque
      plate, downscaled from the 300px master with a Lanczos filter that preserves the round caps
      (FR-817, FR-818)
- [ ] T024 [US2] Implement a minimal ICO writer in `scripts/generate-brand-assets.mjs` — 6-byte
...
- [ ] T025 [US2] Add the `<link rel="icon">` (16 and 32) and `<link rel="apple-touch-icon">` (180)
      tags to `apps/web/index.html` (FR-813, FR-816)
- [ ] T026 [US2] Confirm `<title>` and the description meta in `index.html` are **unchanged** —
      FR-049's single branding constant remains the source of the product name (FR-819)
- [ ] T027 [US2] Tune the small-size fill fraction by eye against the board's own 32/24/16 scale
...
- [ ] T028 [US5] Create `apps/web/src/app/icons.ts` holding the icon declarations as pure data
      (`src`, `sizes`, `type`, `purpose`), following the `branding.ts` precedent that
      `vite.config.ts` already imports from (research R5)
- [ ] T029 [US5] Point `apps/web/vite.config.ts` at that module so the manifest has **one** source
...
- [ ] T029a [US5] **Declare the apple-touch icon in the manifest** alongside the existing three, at
...
- [ ] T030 [US5] Create `apps/web/tests/unit/icon-declarations.test.ts` asserting every declared
      `src` resolves to a file under `apps/web/public` (FR-832, C1.1)
- [ ] T031 [US5] Add a dependency-free PNG dimension reader (IHDR: width at byte 16, height at 20,
      big-endian) and assert every file's **actual** dimensions equal its declared `sizes` (FR-833)
- [ ] T032 [US5] Extend the test to parse `apps/web/index.html` for `rel="icon"` and
      `rel="apple-touch-icon"` hrefs and apply the same two checks (C2.1, C2.2)
- [ ] T033 [US5] Assert the declarations contain **no** `purpose: "monochrome"` (FR-841) and that
      the test needs **no allow-list** to pass on what this feature ships (FR-834)
- [ ] T034 [US5] **Prove the gate fails**: temporarily remove a declared file, run `pnpm test:unit`,
...
- [ ] T035 [US5] Confirm `apps/web/tests/unit/icon-declarations.test.ts` runs inside `pnpm verify`
      via the `unit` project rather than as a manual step (FR-835)
- [ ] T035a [US5] Perform the re-check the icons README's replacement checklist asks for: that
...
- [ ] T036 [US3] Generate `apps/web/public/brand/mark-coral.png` and `mark-navy.png` at ≈96px tall
      with **no plate** (FR-820a) — both from the one matte, painted twice (FR-820b)
- [ ] T037 [US3] Create `apps/web/src/shell/BrandMark.tsx`: a presentational component taking the
      colourway, rendering an `<img>` that is `aria-hidden` with empty alt text, `shrink-0`, sized
      by CSS (FR-820, FR-821)
- [ ] T038 [US3] Add the mark to `apps/web/src/shell/DesktopRail.tsx` beside the product name, in
...
- [ ] T039 [US3] Add the mark to `apps/web/src/shell/TopBar.tsx` in the **navy** colourway, visible
...
- [ ] T040 [US3] Size and constrain the top-bar mark so that at 320px the conference switcher, the
...
- [ ] T041 [P] [US3] Add the stacked mark above the heading in `apps/web/src/auth/SignInScreen.tsx`
      (FR-826) — the only screen whose `<h1>` is the product name
- [ ] T042 [P] [US3] Same in `apps/web/src/app/auth/SignUp.tsx`
- [ ] T043 [P] [US3] Same in `apps/web/src/app/auth/Verify.tsx` — heading text
      `Verify your email address` stays exactly as it is
- [ ] T044 [P] [US3] Same in `apps/web/src/app/auth/ResetRequest.tsx`
- [ ] T045 [P] [US3] Same in `apps/web/src/app/auth/ResetPassword.tsx`
- [ ] T046 [US3] Component test: each surface references the **expected colourway asset** — light
...
- [ ] T047 [US3] Component test: every existing accessible name, heading and landmark is unchanged,
      and the mark is **not** announced (FR-821, SC-807)
- [ ] T047a [US3] Unit test: each colourway meets **≥ 3:1 non-text contrast (WCAG 1.4.11)** against
...
- [ ] T048 [US3] Extend `e2e/responsive.spec.ts`: **exactly one** mark is visible at each width band
      — and specifically **not two** at desktop (FR-824, SC-805)
- [ ] T049 [US3] Extend `e2e/responsive.spec.ts`: with the mark present, no destination overflows
...
- [ ] T050 [US3] Run `pnpm test:a11y` and confirm the accessibility suite passes unchanged
...
- [ ] T051 [US1] Create `e2e/support/capture-screenshots.ts` driving the existing Playwright stack —
...
- [ ] T052 [US1] Capture narrow and wide screenshots of real current surfaces into
      `apps/web/public/screenshots/` and commit them (FR-815a, FR-815b)
- [ ] T053 [US1] Review every captured image for personal data. Seeded fixture data only — no real
...
- [ ] T054 [US1] Add `'screenshots/**'` to `injectManifest.globIgnores` in `apps/web/vite.config.ts`
      beside the existing `'**/*.map'` (FR-815d)
- [ ] T055 [US1] Declare the screenshots in the manifest with `sizes`, `type` and `form_factor`
      (C1 shape), now that the files exist
- [ ] T056 [US1] Assert exclusion: build, then confirm `apps/web/dist/sw.js` contains **zero**
...
- [ ] T057 [US4] Verify determinism: run `node scripts/generate-brand-assets.mjs` on a clean tree
...
- [ ] T058 [US4] Verify the loud failure: move the board aside, run the script, confirm `scripts/generate-brand-assets.mjs`
      names what it expected and emits **no** asset; restore (FR-807)
- [ ] T059 [US4] Read `scripts/generate-brand-assets.mjs` as a reviewer would and confirm every
...
- [ ] T060 [US6] Delete `scripts/generate-provisional-icons.mjs` (FR-836)
- [ ] T061 [US6] Grep the repository for references to `scripts/generate-provisional-icons.mjs`
      and fix every one, `apps/web/public/icons/README.md` included (FR-836)
- [ ] T062 [US6] Measure the total added **precached** weight of `apps/web/public/` and record it
      as a number
...
- [ ] T063 [US6] Rewrite `apps/web/public/icons/README.md` (FR-837): the brand source, the
...
- [ ] T063a [US6] **Book** the deferred work as named follow-ups rather than leaving it noted in
...
- [ ] T064 [P] [US6] Confirm the governance and planning documents corrected during the amendment
...
- [ ] T065 Run `pnpm verify` at the repository root, end to end, and confirm all ten gates are
      green — including that the asset budget has **not** moved (FR-843, SC-812) and the
      colour-literal count has **not** risen (SC-811)
- [ ] T065a Verify this feature's four absences, each cheap and each the kind that stops being true
...
- [ ] T068 Walk **scenario 7** — the shell at 320px, tablet and desktop. Position, size and
      colourway. This is register entry 4 territory and the reason it is escalated
- [ ] T069 Walk **scenario 8** — all five authentication screens carry the mark in the stacked
      arrangement, headings unchanged (SC-808)
- [ ] T070 Walk **scenario 9** — screen reader: the mark is silent, everything else is unchanged
- [ ] T071 Update `spec.md` Status to reflect delivery, and record in the feature directory which

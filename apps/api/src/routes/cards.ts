...
export const cardRoutes = async (app: FastifyInstance): Promise<void> => {
  ...
  app.get(
    '/cards/shared',
    {
      ...
      schema: {
        tags: ['network'],
        summary: 'Cards the attendee has given away',
        description:
          'Read-only. **There is no `DELETE` here and there will not be one** (FR-618): a card cannot be recalled, because you cannot un-give what somebody already holds. Deliberately thinner than the held list — sharing your card does not entitle you to the recipient\'s profile, which is what "one-directional" means in practice.',
        security: [{ sessionCookie: [] }],
        response: {
          200: {
            type: 'object',

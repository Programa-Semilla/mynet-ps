...
/**
 * T023a (016) — **every password field in the administrative site is a `PasswordField`, and
 * nothing else asserted that** (FR-1014, FR-1016, SC-1004).
 *
 * ═════════════════════════════════════════════════════════════════════════════════════════════
 * **THE TWIN COMPONENT TESTS PROVE THE COMPONENT IS CORRECT. THEY PROVE NOTHING ABOUT THE
 * SCREENS.**
 *
 * `password-field.test.tsx` — here and in `apps/web` — asserts that `PasswordField` reveals,
 ...
 *
 * None of that is a claim about the **screens**. SC-1004 is *"every password field across both
 * products"*, and a screen rendering a bare `<input type="password">` satisfies every existing
 * test while defeating it — silently, and most plausibly on the *next* screen somebody adds,
 * because reaching for an `<input>` is what writing a form normally is.
 *
 * **It costs more here than in MyNet.** This product has three password inputs and all three are
 * on the two screens guarding the tier that reads the report queue: administrative sign-in, and
 * the forced replacement of a credential that came from an environment variable sitting in a
 * `.env` on the host. A field somebody cannot check before submitting is worst exactly where
 * retyping it is hardest.
 * ═════════════════════════════════════════════════════════════════════════════════════════════
 *
 * ─────────────────────────────────────────────────────────────────────────────────────────────
 * **This file has a twin: `apps/web/tests/unit/password-field-usage.test.ts`.** Two products, two
 * `PasswordField` implementations, two guards — the same arrangement, for the same reason:
 * `apps/admin` depends on `@mynet/data` and `@mynet/config` only, so there is no shared package
 * for either the component or its guard to live in. A single guard scanning both trees would have
 * to reach across an app boundary that exists on purpose, and `no-platform-dependency.test.ts`
 * one file over is what that boundary is made of.
 * ─────────────────────────────────────────────────────────────────────────────────────────────
 */
...
const THE_COMPONENT = join('app', 'ui', 'PasswordField.tsx')
...
describe('the administrative site has exactly one password input (SC-1004)', () => {
  ...
  it('names the type in `PasswordField` and nowhere else', () => {
    expect(
      namesTheType(),
      'A password input was rendered outside `app/ui/PasswordField.tsx`. SC-1004 is "every ' +
        'password field across both products", so a bare <input type="password"> is not a ' +
        'smaller version of the feature — it is a field with no reveal control, no ' +
        'keyboard-operable toggle, and no assertion anywhere that it behaves like the other ' +
        'four. Import `PasswordField` instead; if this screen genuinely cannot, that is a ' +
        'decision and this file is where it gets recorded.',

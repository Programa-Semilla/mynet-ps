...
delivered is retracted, and no work performed under 4.0.0 or 4.1.0 is invalidated. It is the same
test 3.1.0 applied to itself when it brought delivery into scope at all, and for the same reason it
is **not PATCH**: widening a trigger set changes what the product may do to somebody's phone, and
that is not a clarification of what it already did.
...
Register changes:
  - 27. OPENED. Whether an attendee may suppress content in notifications. **Promoted** from a
    deferral recorded in prose since 3.1.0 to a numbered entry, because 4.2.0 makes it apply to a
    second content type and it has now gone undecided across two amendments. Blocks nothing.
  - 28. OPENED. Speakers are personal data about people who are not attendees, and 4.2.0 makes them
...
- **Conference content is seeded; profiles are attendee-authored.** Events, sessions, tracks, rooms,
  ...
  *Amended 4.0.0 by A4.* This clause previously read: "Seed data MUST NOT become a route around the
  ...
  - **A profile is still not conference content.** Each attendee authors their own, and **no
    ...
    *Sharpened 4.2.0.* **A speaker is conference content; a speaker is not an attendee.** 014 makes
  ...
  *Extended 4.2.0 by N4 and N5.* Authoring is live, and it has one prohibition attached:
...
*Extended 4.2.0 by N1 and N2 — the trigger set becomes two, and the second is bounded by a named set
...
3.1.0 it was excluded outright; from 3.1.0 to 4.2.0 there was exactly one trigger. The boundary is
...
- **There are exactly TWO triggers, and they are enumerated here.**
  ...
  2. **A material change to a session the attendee has SAVED.** *4.2.0, N1.* **Material means
...
- **The notification bell MUST NOT be reproduced, and neither MUST an in-app notification centre.**
  ...
  *Extended 4.2.0 by N2.* 014 marks a changed saved session **on the row itself**, in Agenda and on
...
- **Notification content is personal data on a lock screen, and that consequence is ACCEPTED rather
  ...
  *Extended 4.2.0 by N1.* The same consequence now reaches a **second** content type: a
  ...
  amendments, it is promoted from a deferral in prose to **register entry 27**. Recording it a
...
*Rationale for the second trigger (4.2.0)*: 014 lets an organizer change a programme attendees are
...
- **Conference organizer.** An attendee promoted by a platform operator. Authority reaches **only
  ...
  *Extended 4.2.0 by N3.* **A conference organizer MAY CREATE a conference, and is assigned to the
...
- **27. Whether an attendee may suppress content in notifications.** **Promoted** from a deferral
  that has sat in prose since 3.1.0 — it was recorded there as "accepted rather than solved" and
  never given a number. 4.2.0 makes it apply to a **second** content type: a saved-session
...
- **28. Speakers are personal data about people who are not attendees.** A speaker row carries a
  real person's name, title and company. This is not new — the rows have been seeded since 002 — but
  4.2.0 makes them **organizer-authored**, which moves responsibility from a reviewed commit to a
...
**Version**: 4.2.0 | **Ratified**: 2026-08-04 | **Last Amended**: 2026-08-12

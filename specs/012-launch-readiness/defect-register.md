...
| DR-6 | Part B (duplicate promote) | PromoteDialog | The 409 for an already-promoted attendee rendered "That could not be completed." while the server wrote "That attendee already organizes this conference." — the explained-refusal-swallowed class. | defect | dialog renders the specific sentence for refused on this route (a 409 here has exactly one documented meaning, describing the caller's own duplicate act) | **awaiting re-walk** — the earlier 're-walked pass (A4 row 2)' was wrong: the fix was never deployed until 2026-09-25 (see A4's correction row) |

...
- [ ] T061 Plant **three defects** — one layout, one copy, one refusal message — in a throwaway build deployed to UAT, recorded as *base commit + named seed patch*. **Not `develop`** (FR-1127). A seed any automated gate catches first was the wrong seed — replace it with one no gate can see (edge case 6)

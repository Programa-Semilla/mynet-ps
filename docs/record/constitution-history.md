...
(`.specify/memory/constitution.md`) is authoritative.

**6.0.0 (2026-09-24) rewrote the constitution into Spec Kit's intended shape**: numbered, testable
principles and rules only, consumed by `/speckit-plan`'s Constitution Check and `/speckit-analyze`.
Everything else the document had accumulated is preserved here **verbatim**:

- **Part 1** — every Sync Impact Report, 5.4.0 back to 1.0.0. From 6.0.0 on, an amendment's report
  is its PR description, and its summary is prepended to Part 0 below (constitution G.2).
- **Part 2** — the Open Questions Register as it stood in 5.4.0. The live register is now
  `docs/governance/open-questions.md`.
- **Part 3** — the full binding text of 5.4.0 (principles, constraints, workflow, governance) with
  all its rationale, amendment annotations and "why this was granted" reasoning. When a 6.0.0 rule's
  reason is not obvious, it is here.

## Part 0 — Amendments since 6.0.0 (newest first)

### 6.0.0 — 2026-09-24 — restructured to Spec Kit's purpose

MAJOR: governance changes. The register and the decision log leave the constitution (closing a
question no longer needs an amendment unless it changes a rule); the Sync Impact Report is no longer
committed into the file, following Spec Kit's own instruction that it is "temporary scratch
material"; and every rule is renumbered. Rules are restated as numbered MUST/SHOULD statements with
no history. Discharged one-off obligations were dropped (the `AdminShell` third layout and the
widened attendee-restriction guard, both delivered in #30); owner-personal obligations moved to the
open-questions register. Added: Principle X ("Invariants Are Enforced, Not Asserted"), distilled from
the defects every deep review since 008 found, and an explicit Constitution Check list for plans.
Size: 304KB → ~40KB.

**Rules that are new to the constitution, listed so ratification is deliberate rather than
incidental.** None is new to the project: each was an invariant recorded in `CLAUDE.md` (under
"changing one is a decision, not a refactor"), a shipped feature requirement, or a lesson a deep
review recorded. Promoting them makes `/speckit-plan` and `/speckit-analyze` enforce them.

- **Principle X in full** (X.1–X.7): guards over prose, tested absences, fail-by-existing coverage,
  false headers as defects, distinct refusal codes, faithful test doubles, prose verified by reading.
- **VIII.10** (refusals indistinguishable from not-found unless the reason concerns the reader),
  from 007/008/009.
- **VII.1** WebKit and Firefox e2e and the build-output audit (012, 010); **VII.3** a skipped check
  is a defect (010's review); **VII.5** narrow, never weaken, a noisy guard (014 D14); **VII.8**
  by-hand walks before production (012).
- **V.4** configuration-selected adapters and lint confinement (007, 011); **V.5** per-line lint
  exemptions (016); **V.2** "unrepresentable" as a SHOULD (016).
- **VI.3** writes refused offline, never queued; **VI.2** retrieval time shown (005 onward).
- **IV.4** "including inside a scroll container" (generalised from the discharged `AdminShell`
  carve-out).
- **A.2** no editing an applied migration; **A.5** locks inside transactions, pool timeouts (009,
  013, 016); **A.13** readiness gating (006).
- **P.7** engagement check under lock; **P.10** email unique across both tables (013 FR-918);
  **P.17** recipient discoverable (016 FR-1053); **P.19–P.20** proposal/double-booking/withdrawal
  rules (008); **P.21–P.23** the Messages rules (007); **P.28** votes counted at read time (009);
  **P.29** nothing time-driven dispatches (014); **P.31–P.32** permission and subscription rules
  (007); **P.36–P.39** operator without credential, type-level tiers, write classification, audit
  atomicity (013, #30); **P.45** equality check on the upscale exception (016).
- **Workflow and governance**: I.5 (no re-litigating a recorded decision, generalised from D5), W.3
  (check in-flight branches before claiming any number), W.6 deletion/export tasks, W.7 review
  findings and deviations files, G.2 ratification-by-merge and the PR-as-Sync-Impact-Report
  convention, G.3's withdraw-is-MAJOR / narrow-is-MINOR test (practised since 5.3.0), G.5 closing a
  question needs a recorded decision.

A completeness audit compared every normative statement in 5.4.0 against 6.0.0 before this was
proposed; the six rules and eight wordings it found lost or weakened were restored.
...
The Open Questions Register exactly as it stood in 5.4.0, before it was restructured into one
list in number order and moved to `docs/governance/open-questions.md`. Original entry texts, escalations and annotations live here.
...
## Part 3 — Binding text as of 5.4.0 (verbatim)
